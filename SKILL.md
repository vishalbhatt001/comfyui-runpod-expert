...
**Pre-submit checklist, before ever calling `/prompt`** (this exact warning appears independently in every one of Comfy Org's own generation command docs — image, video, 3D, audio, background-removal, upscale — so it's evidently a common real mistake, not a theoretical one): confirm the graph has **both** (a) at least one input node actually carrying the user's intent (`CLIPTextEncode` for a prompt, `LoadImage`/`LoadAudio` for a source file) and (b) at least one *save/output* node wired to the final tensor (`SaveImage`, `SaveVideo`, `SaveAudioAdvanced`, etc.) — not just a node that *produces* a result. A graph that generates but never saves runs to completion, costs the same GPU-minutes, and returns nothing retrievable. Silent, expensive, and easy to miss when building a graph node-by-node rather than from a template.
...
- **`SaveAudioMP3` is deprecated** (confirmed both by direct source inspection — marked `DEPRECATED` in `comfy_extras/nodes_audio.py` alongside `SaveAudio`/`SaveAudioOpus` — and independently by Comfy Org's own `generate-audio.md`, which names `SaveAudioAdvanced` as the current replacement, requiring a `format` input alongside `audio`/`filename_prefix`). It still worked when built (deprecated ≠ removed, and it's what the official reference workflow itself used at the time), but a future ComfyUI update could remove it outright — worth migrating `ace-music-studio` and `music-video-generator` to `SaveAudioAdvanced` next time either is touched, rather than waiting for it to break.
...
**Pricing (checked 2026-08-27, will drift — reverify before quoting):** `search_templates`/`search_models`/`search_nodes` are **free**, no subscription needed, just a Comfy account — this is the part actually worth using. **Actually generating** on Comfy Cloud requires a paid subscription (Standard $20/mo · Creator $35/mo · Pro $100/mo · Team $700/mo, each with a monthly credit allotment; 5 free trial runs for new users). That billing is entirely for compute on *Comfy's own* cloud GPUs and has nothing to do with a RunPod deployment — running the search tools, or running ComfyUI locally/on RunPod, doesn't touch this subscription at all. Net recommendation: worth installing for the free search tools alone; RunPod remains the cheaper path for actual generation given everything already built here.
